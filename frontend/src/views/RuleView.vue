    ...
    <template v-if="rules">
      <div class="page-card mb">
        ...
        <el-table :data="rules.bmi" border size="small">
          <el-table-column label="BMI 范围">
            <template #default="{ row }">{{ fmtRange(row.min, row.max) }}</template>
          </el-table-column>
      ...
      <div class="page-card mb">
        ...
        <el-table :data="rules.personalHistory" border size="small">
          ...
          <el-table-column label="具体疾病"><template #default="{ row }">{{ row.diseases.join('、') }}</template></el-table-column>
      ...
      <div class="page-card mb">
        ...
        <el-table :data="rules.age" border size="small">
          <el-table-column label="年龄范围"><template #default="{ row }">{{ fmtAge(row.min, row.max) }}</template></el-table-column>
          <el-table-column prop="label" label="分类" />
          <el-table-column label="风险加分"><template #default="{ row }"><b class="score">+{{ row.score }}</b></template></el-table-column>
        </el-table>
      </div>
      ...
      <div class="page-card mb">
        ...
        <el-table :data="rules.familyHistory" border size="small">
          ...
          <el-table-column label="具体病史"><template #default="{ row }">{{ row.diseases.length ? row.diseases.join('、') : '（待历史数据挖掘填充）' }}</template></el-table-column>
          <el-table-column label="风险加分" width="100"><template #default="{ row }"><b class="score">+{{ row.score }}</b></template></el-table-column>
        </el-table>
      </div>
      ...
      <div class="page-card mb">
        ...
        <el-table :data="rules.levels" border size="small">
          ...
          <el-table-column label="分数范围" width="120"><template #default="{ row }">{{ scoreRange(row) }}</template></el-table-column>
          <el-table-column prop="result" label="核保结论" />
          <el-table-column prop="claimProb" label="历史理赔概率" width="130" />
          <el-table-column prop="premiumAdjustment" label="加费系数" width="90" />
        </el-table>
      </div>
    </template>
  </div>
